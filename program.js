      ...
      return {
        ...
        guess: function () {
          ...
          this.restWordList = this.restWordList.filter(word => {
            ...
            let subRes = [word]
            if (execRes.length > 1) {
              subRes = execRes.slice(1, execRes.length) // 取出各缺口的匹配结果
            }
            subRes.forEach((subLetters, index) => {
              let stator = positionCounter[index]
              if (!stator) {
                stator = {
                  max: { letter: '', count: 0 },
                  letterCount: 0,
                ...
                positionCounter[index] = stator
              }
              const { max, counter } = stator
              subLetters.split('').forEach(letter => {
